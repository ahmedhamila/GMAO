...
class Ui_Dialog(object):
    def __init__(self,mainWindowSelf,id,EquipementDLG,EquipementUI) -> None:
        self.mainWindowSelf=mainWindowSelf
        self.id=id
        self.EquipementDLG=EquipementDLG
        self.EquipementUI=EquipementUI
    ...
    def updateEquipement(self):
        ...
        record=(designation,role,fabriquant,dateFabriquation,dateMiseEnMarche,reference)
        
        EquipementServices.updateEquipement(record)
        
        
        self.showDialog('Success',"Equipement modifié avec succé",True)
        self.EquipementUI.fetchRows()
        self.mainWindowSelf.stackedWidget.setCurrentWidget(self.EquipementDLG)
        

    def initialisationEquipement(self):

        status,record = EquipementServices.getEquipement(self.id)
        if status :
            self.lineEditReference.setText(record[0][0])
            self.lineEditReference.setReadOnly(True)
            self.lineEditDesignation.setText(record[0][1])
            self.lineEditRole.setText(record[0][2])
            self.lineEditFabriquant.setText(record[0][3])
            dateF=str(record[0][4]).split("-")
            dateM=str(record[0][5]).split("-")
            print(dateF,dateM)
            self.dateEditFabriquation.setDate(QDate(int(dateF[0]),int(dateF[1]),int(dateF[2])))
            self.dateEditMiseEnMarche.setDate(QDate(int(dateM[0]),int(dateM[1]),int(dateM[2])))
...
"};")
        ...
        self.buttonEnvoyer.clicked.connect(self.updateEquipement)
